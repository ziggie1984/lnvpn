...
io.on('connection', (socket) => {
  ...
  socket.on('checkInvoice', (clientPaymentHash) => {
    ...
    checkInvoice(clientPaymentHash)
      ...
      .catch((error) => {
        ...
        logDim(
          `no way to recover this state in a secure manner | server crashed potentially or client already got the Config from the server`
        );
      });
  });
  ...
  socket.on('sendEmail', (emailAddress, configData, date) => {
    sendEmail(emailAddress, configData, date).then(
      (result) => DEBUG && console.log(result)
    );
  });

  socket.on('disconnect', () => {
    DEBUG && console.log(`User disconnected with ID: ${socket.id} `);
